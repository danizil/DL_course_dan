...
class LeakyReLU(Layer):
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        # Danny: this broadcast multiplication works dout*(...) because the
        #        derivative dout/dx is a diagonal matrix 
        dx = dout*((x > 0).long() + self.alpha*((x<=0).long()))
        # ========================
...
class Sigmoid(Layer):
    ...
    def forward(self, x, **kw):
        ...
        out_naive = 1/(1+torch.exp(-x))
        # A: Bad!! so bad..
        # A: prevent overflow. Even with values ranging from -5 to 5 we get 
        #    overflow! 
        # TODO: change the global maximum to a per-sample maximum
        # m = torch.max(x)
        # ex = torch.exp(x - m)
        # out_norm = ex/(torch.exp(-m) + ex)
        out = out_naive
    ...
    def backward(self, dout):
        ...
        # A: nice trick with the sigmoid derivative:
        doutdx = out*(1 - out)
        dx = dout* doutdx
...
class TanH(Layer):
    ...
    def forward(self, x, **kw):
        ...
        #  Save whatever you need into grad_cache.
        # ====== YOUR CODE: ======
        # A: since tgh(x) = 2(sigma(2x) - 1/2):
        s = Sigmoid()
        out = 2*(s.forward(2*x) - 1/2)
        self.grad_cache['out'] = out
        # ========================
    ...
    def backward(self, dout):
        ...
        # TODO: Implement gradient w.r.t. the input x
        # ====== YOUR CODE: ======
        # A: trying dx = 4*self.s.backward(dout) #almost works... diff 0.012
        out = self.grad_cache['out']        
        dx = dout*(1 - out**2)
        # ========================
...
class CrossEntropyLoss(Layer):
    ...
    def forward(self, x, y):
        ...
        xy = torch.gather(x, 1, y[:, None])
        # mask = torch.ones_like(x).scatter_(1, y.unsqueeze(1), 0.)
        # x_rest = x[mask.bool()].view([x.shape[0], x.shape[1]-1]) # these were a mistake
                                                                   # but i really need this
                                                                   # for future reference

        logsumexp = torch.log(torch.sum(torch.exp(x), dim=1))
        loss_vec = logsumexp - xy
        loss = torch.sum(loss_vec)/N**2
        self.grad_cache['out'] = loss 
        # ========================
    ...
    def backward(self, dout=1.0):
        ...
        # ====== YOUR CODE: ======
        sm = torch.exp(x)
        sm /= sm.sum(axis=1, keepdims=True) #softmax
        sm[range(N), y] -= 1
        dx = dout*sm/N
        # ========================
...
class Dropout(Layer):
    ...
    def forward(self, x, **kw):
        ...
        # ====== YOUR CODE: ======
        # different mask for every layer? no but each layer has it's 
        # own mask
        if self.training_mode:
            mask = torch.empty_like(x).uniform_(0,1)
            mask = (mask > self.p).int()
            out = x * mask
...
class Sequential(Layer):
    ...
    def forward(self, x, **kw):
        ...
        for i, layer in enumerate(self.layers):
            x = layer(x, **kw)
        out = x
        # self.grad_cache['out'] = out for debugging
        # ========================        
...
class MLP(Layer):
    ...
    def __init__(
        ...
        **kw,
        ...
        # A: things to take from here: if an object changes when another object changes, 
        #    it could be a "by reference" bug
        NLS = [ReLU() if activation == 'relu' else Sigmoid() for i in range(len(hidden_features))]
        layers += [Linear(in_features, hidden_features[0], **kw), NLS[0], Dropout(dropout)]
        for i in range(1, len(hidden_features)):
            layers += [Linear(hidden_features[i-1], hidden_features[i], **kw), NLS[i], Dropout(dropout)]
