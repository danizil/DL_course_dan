...
from hw2.layers import CrossEntropyLoss
...
def mlp_experiment(
    ...
    hp_optim = dict(lr=0.00921, 
            weight_decay=0.005, 
            momentum=0.9, 
            loss_fn=CrossEntropyLoss())
    # define the model and classifiers
    ...
    def init_weights(m):
        if isinstance(m, torch.nn.Linear):
            torch.nn.init.normal_(m.weight, mean=0, std=wstd)
    ...
    valid_acc = fit_result.test_acc[-1] # TODO: maybe not the last but the first
    
    # pick thresh from validation
