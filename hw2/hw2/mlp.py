...
class MLP(nn.Module):
    ...
    def __init__(
        ...
        for i, _ in enumerate(all_dims[:-1]):
            self.layers.append(nn.Linear(in_features=all_dims[i], out_features=all_dims[i+1]))
            if isinstance(nonlins[i], str):
                self.layers.append(ACTIVATIONS[nonlins[i]]())
            else:
                self.layers.append(nonlins[i])
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        out = self.model(x)
        assert (self.out_dim == out.shape[1])
