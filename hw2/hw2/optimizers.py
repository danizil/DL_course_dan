...
class MomentumSGD(Optimizer):
    ...
    def step(self):
        for p, dp in self.params:
            ...
            # ====== YOUR CODE: ======
            #   could be a problem with v having to belong to a layer
            dp += self.reg*p
            self.vs[self.iter] = self.momentum*self.vs[self.iter] - self.learn_rate*dp
            p += self.vs[self.iter]
            self.iter += 1
        self.iter = 0 
            # ========================
...
class RMSProp(Optimizer):
    ...
    def step(self):
        for p, dp in self.params:
            ...
            # parameters tensor.
            # ====== YOUR CODE: ======
            # test this: calculate two steps and track in the debugger
            decay = self.decay
            lr = self.learn_rate
            r = self.rs[self.iter]
            dp += self.reg*p
        
            r = decay*r + (1 - decay)*dp**2
            adj_lr =  lr/torch.sqrt(r + self.eps)
            p -= adj_lr*dp
