...
class Classifier(nn.Module, ABC):
    ...
    def __init__(self, model: nn.Module):
        ...
        # ====== YOUR CODE: ======
        self.probability_fn = nn.functional.softmax
        # ========================
    ...
    def predict_proba_scores(self, z: Tensor) -> Tensor:
        ...
        # ====== YOUR CODE: ======
        return self.probability_fn(z, dim=1)
        # ========================
...
class ArgMaxClassifier(Classifier):
    ...
    def _classify(self, y_proba: Tensor):
        ...
        # ====== YOUR CODE: ======
        classified = torch.argmax(y_proba, dim=1)
        return classified
        # ========================
...
class BinaryClassifier(Classifier):
    ...
    def _classify(self, y_proba: Tensor):
        ...
        #  Output should be a (N,) integer tensor.
        # ====== YOUR CODE: ======
        classified = (y_proba[:, self.positive_class] >= self.threshold).int()
        assert classified.shape[0] == y_proba.shape[0]
        return classified
        # ========================
...
def plot_decision_boundary_2d(
    ...
    # create a mesh with the step size and plot bounds
    ylim = ax.get_ylim()
    xlim = ax.get_xlim()

    xs = torch.arange(xlim[0], xlim[1], dx)
    ys = torch.arange(ylim[0], ylim[1], dx)
    x1_grid, x2_grid = torch.meshgrid(xs, ys)
    coords = torch.stack([x1_grid, x2_grid], dim=2)
    coord_list = torch.flatten(coords, start_dim=0, end_dim=1)
    # run the points through the classifier
   
    # y_hat = classifier.classify(coords.rot90(dims=[1,2]))
    y_hat = classifier.classify(coord_list).unsqueeze(dim=1)
    coords_lables = torch.hstack([coord_list, y_hat])
    coords_lables_orig_shape = coords_lables.view(*x1_grid.shape, 3)
    
    assert (coords_lables_orig_shape[:,:,0] == x1_grid).all()
    assert (coords_lables_orig_shape[:,:,1] == x2_grid).all()

    y_hat = coords_lables_orig_shape[:,:,2]
    # ========================
...
def select_roc_thresh(
    ...
    # ====== YOUR CODE: ======
    import numpy as np
    #classify x and get a list of the winner score
    y = y.detach().numpy()
    cls_probs = classifier.predict_proba(x).detach().numpy()

    prob_2_b_pos = cls_probs[:, 1]
    fpr, tpr, thresh = roc_curve(y, prob_2_b_pos)
    
    dists_from_01_sq = (tpr - 1)**2 + fpr**2
    # optimal_thresh = np.min(dists_from_01_sq) 
    optimal_thresh_idx = np.argmin(dists_from_01_sq)
