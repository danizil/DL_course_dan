...
def random_labelled_image(
    ...
    label = torch.randint(num_classes-1, (1,)).item()
    # ========================
    return image, label
...
class RandomImageDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> Tuple[Tensor, int]:
        ...
        with torch_temporary_seed(index):
            image, label = random_labelled_image(self.image_dim, num_classes=self.num_classes)
        return image, label
        # ========================
...
class ImageStreamDataset(IterableDataset):
    ...
    def __iter__(self) -> Iterator[Tuple[Tensor, int]]:
        ...
        while True:
            ...
            self.image = image
            self.label = label
            yield iter((self.image, self.label))
        # ========================
...
class SubsetDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if index >= self.subset_len:
            raise IndexError("index exceeds subset length")
